...
export function KasbonSettlementTab() {
  ...
  return <div className="space-y-3"><div className="flex justify-end"><button disabled={!rows.length} onClick={exportFile} className="inline-flex items-center gap-2 rounded-md bg-primary px-3 py-2 text-sm text-primary-foreground disabled:opacity-50"><Download className="w-4 h-4" /> {t("kasbonsettle.exportBtn")}</button></div><div className="rounded-xl border border-border overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted text-left"><tr><th className="p-3">{t("kasbonsettle.colRecipient")}</th><th>{t("kasbonsettle.colAccount")}</th><th>{t("kasbonsettle.colRider")}</th><th>{t("kasbonsettle.colPayroll")}</th><th className="text-right">{t("kasbonsettle.colCollected")}</th><th>{t("kasbonsettle.colStatus")}</th></tr></thead><tbody>{rows.length ? rows.map(r => <tr key={r.id} className="border-t border-border"><td className="p-3 font-medium">{r.recipient}</td><td>{r.bank} · {r.account}</td><td>{r.rider}</td><td>{r.period}</td><td className="text-right font-semibold">{rp(r.amount)}</td><td className={r.status === "Siap ditransfer" ? "text-success" : "text-warning"}>{r.status}</td></tr>) : <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">{t("kasbonsettle.emptyState")}</td></tr>}</tbody></table></div></div>;
}

export function KasbonLunasTab() { const { t } = useT(); return <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">{t("kasbonlunas.emptyState")}</div>; }
