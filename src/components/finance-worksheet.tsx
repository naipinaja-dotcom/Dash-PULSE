...
export function FinanceWorksheet({ runId, run }: { runId: string; run?: Run }) {
  ...
  useEffect(() => {
    if (run?.client_id) {
      getClientExportTemplate(run.client_id).then((cols) =>
        setEnabledCols(new Set(cols ?? ALL_EXPORT_COLUMN_KEYS)),
      );
    } else {
    ...
    (async () => {
      ...
      try {
        ...
        const { data: details, error: e1 } = await sb
          ...
          .select(
            "id:detail_id, rider_id, client_id, delivery_count, gross_earning, net_pay, remarks, rider_name, rider_employee_id, client_name, incentive",
          )
          .eq("run_id", runId);
        if (e1) throw e1;
        ...
        const clientIds = new Set<string>(
          (details ?? [])
            .map((d: { client_id: string | null }) => d.client_id)
            .filter(Boolean) as string[],
        );
        const allSchemes = await listPricingSchemes();
        const riderSchemes = allSchemes.filter(
          (s) => s.scheme_for === "rider" && (s.client_id === null || clientIds.has(s.client_id)),
        );
        ...
        // Skema rider (delivery) — dipakai buat replay "Rule Area kepilih" per
        // baris Kiriman (lihat delivByRider di bawah), sama pola persis kayak
        // resolveAttScheme di atas. resolveSchemeForCity/resolveAreaPricingRule
        // di-reuse APA ADANYA dari pricing-calc.ts (mesin asli) biar label ini
        // gak mungkin beda dari yang beneran dipakai ngitung fee pas payroll
        // jalan — ini cuma buat TAMPILAN, gak pernah nulis ulang `fee`.
        const deliverySchemes = riderSchemes.filter((s) => s.category === "delivery");
        const delivSchemesByClient = new Map<string, typeof deliverySchemes>();
        const globalDeliverySchemes: typeof deliverySchemes = [];
        for (const s of deliverySchemes) {
          if (s.client_id === null) globalDeliverySchemes.push(s);
          else
            delivSchemesByClient.set(s.client_id, [
              ...(delivSchemesByClient.get(s.client_id) ?? []),
              s,
            ]);
        }
        const resolveAreaRuleLabel = (
          clientId: string | null,
          city: string | null,
        ): string | null => {
          if (!clientId) return null;
          const candidates = [
            ...(delivSchemesByClient.get(clientId) ?? []),
            ...globalDeliverySchemes,
          ];
          if (candidates.length === 0) return null;
          const scheme = resolveSchemeForCity(candidates, city, clientId);
          const acp = scheme?.params?.area_city_pricing;
          if (!acp?.enabled) return null;
          const rule = resolveAreaPricingRule(acp, city);
        ...
        for (let i = 0; i < detailIds.length; i += 200) {
          ...
          const { data: deds, error: e2 } = await sb
            ...
            .select(
              "detail_id, amount, deduction_types(name, code), kasbon_recipient_id, kasbon_recipients(name, bank_name, account_number, account_holder, no_transfer_needed)",
            )
            .in("detail_id", chunk);
          if (e2) throw e2;
          for (const d of deds ?? []) {
        ...
        const grossByDetail = new Map(
          (details ?? []).map((d: { id: string; gross_earning: number }) => [
            d.id,
            Number(d.gross_earning),
          ]),
        );
        const riderNameByDetail = new Map(
          (details ?? []).map((d: { id: string; rider_name?: string | null }) => [
            d.id,
            d.rider_name ?? "(tanpa nama)",
          ]),
        );
        const kasbonAllocations = allocateKasbonByRecipient(
          grossByDetail,
          kasbonDeductionRows,
          riderNameByDetail,
        );
        ...
        const [delivs, atts] = await Promise.all([
          ...
          fetchAllRows<{
            ...
            let q = c
              ...
              .select(
                "rider_id, client_id, delivery_date, distance_km, weight_kg, delivery_type, district, city, fee",
              )
          ...
          fetchAllRows<{
            ...
            let q = (c as any)
              ...
              .select(
                "rider_id, client_id, pitstop_name, log_date, clock_in, clock_out, duration_minutes, is_late, is_absent, fee",
              )
        ...
        const compByRowKey = new Map<
          string,
          { base: number; overtime: number; incentiveAmt: number; shiftLabel: string | null }
        >();
        for (const [clientKey, group] of attsByClientKey) {
          ...
          const comp = cfg
            ? calcAttendanceComponent(logs, cfg)
            : group.map(() => ({ daily_base: 0, overtime: 0, incentive: 0 }));
          group.forEach((a, i) => {
            const shift = shifts.length > 0 ? findShiftFor(a.clock_in, shifts) : null;
            compByRowKey.set(`${a.rider_id}|${a.log_date}|${a.clock_in ?? ""}`, {
              base: comp[i].daily_base,
              overtime: comp[i].overtime,
              incentiveAmt: comp[i].incentive,
        ...
        for (const r of delivs) {
          ...
          (delivByRider.get(r.rider_id) ?? delivByRider.set(r.rider_id, []).get(r.rider_id)!).push({
            ...
            city: r.city,
            fee: Number(r.fee) || 0,
            areaRule: resolveAreaRuleLabel(r.client_id, r.city),
          });
          const s = datesByRider.get(r.rider_id) ?? new Set<string>();
          s.add(r.delivery_date);
          datesByRider.set(r.rider_id, s);
        }
        ...
        for (const a of atts) {
          ...
          (attByRider.get(a.rider_id) ?? attByRider.set(a.rider_id, []).get(a.rider_id)!).push({
            date: a.log_date,
            clockIn: a.clock_in,
            ...
            absent: !!a.is_absent,
            fee,
            pitstop: a.pitstop_name,
            clientId: a.client_id,
            shiftLabel: comp?.shiftLabel ?? null,
            base: comp ? comp.base : fee,
            overtime: comp?.overtime ?? 0,
            incentiveAmt: comp?.incentiveAmt ?? 0,
          });
        }

        const built: RiderRow[] = (details ?? []).map(
          (d: {
            id: string;
            rider_id: string;
            ...
            remarks: d.remarks ?? "",
            deliv: (delivByRider.get(d.rider_id) ?? []).sort((a, b) =>
              a.date.localeCompare(b.date),
            ),
            att: (attByRider.get(d.rider_id) ?? []).sort((a, b) => a.date.localeCompare(b.date)),
            held: holdByDetail.get(d.id)?.status === "held",
            holdReason:
              holdByDetail.get(d.id)?.status === "held" ? holdByDetail.get(d.id)!.reason : null,
            isKasbonRow: false,
        ...
        const kasbonRows: RiderRow[] = kasbonAllocations.map((a) => ({
          ...
          total: a.amount,
          remarks:
            `Kasbon dari ${a.riderNames.join(", ")}` +
            (a.noTransferNeeded ? " — rekening internal, gak perlu transfer" : ""),
          deliv: [],
          att: [],
          held: false,
          holdReason: null,
  ...
  const internalKasbonRows = useMemo(
    () => rows.filter((r) => r.isKasbonRow && r.kasbonNoTransfer),
    [rows],
  );
  const t = useMemo(
    () =>
      rows
        .filter((r) => !r.held && !r.kasbonNoTransfer)
        .reduce(
          (s, r) => ({
            order: s.order + r.orderCount,
            fee: s.fee + r.feeRider,
            total: s.total + r.total,
            ded: dedTypes.reduce(
              (m, ty) => ({ ...m, [ty]: (m[ty] ?? 0) + (r.ded[ty] ?? 0) }),
              s.ded,
            ),
          }),
  ...
  const summaryRows = (): Cell[][] => {
    ...
    const body: Cell[][] = rows.map((r) => {
      ...
      if (c.has("deductions"))
        row.push(...dedTypes.map((ty) => (r.isKasbonRow ? "" : (r.ded[ty] ?? 0))));
      if (c.has("total_fee")) row.push(r.total);
      if (c.has("remarks"))
        row.push(r.held ? `[PEMBAYARAN DITAHAN: ${r.holdReason}] ${r.remarks}` : r.remarks);
      return row;
    });
  ...
  const exportExcel = () => {
    const sheets = [
      {
        name: "Rate Card (PKS)",
        rows: rateCards.length
          ? rateCardsToRows(rateCards)
          : [["(tidak ada skema rider untuk client di run ini)"]],
      },
      { name: "Detail", rows: isAttendanceOnly ? attendanceDetailRows(rows) : detailRows() },
      {
        name: "Ringkasan",
        rows: isAttendanceOnly ? attendanceSummaryRows(rows, run) : summaryRows(),
      },
    ];
  ...
  const exportSummaryCSV = () =>
    downloadCSV(
      `ringkasan-${run?.name ?? runId}.csv`,
      toCSV(isAttendanceOnly ? attendanceSummaryRows(rows, run) : summaryRows()),
    );
  const exportDetailCSV = () =>
    downloadCSV(
      `detail-${run?.name ?? runId}.csv`,
      toCSV(isAttendanceOnly ? attendanceDetailRows(rows) : detailRows()),
    );
  ...
  return (
    <>
      ...
      <div className="admin-glass-panel rounded-xl border border-border bg-card mb-4 overflow-hidden">
        ...
        >
          ...
          <span className="text-sm font-semibold">
            {tr("financeWs.rateCardHeader")} ({rateCards.length} {tr("financeWs.schemesUnit")})
          </span>
          <ChevronRight
            className={`w-4 h-4 ml-auto transition-transform ${showRates ? "rotate-90" : ""}`}
          />
        </button>
        {showRates && (
              ...
              rateCards.map((c, i) => (
                <div key={i} className="rounded-lg border border-border overflow-hidden">
                  <div className="admin-glass-strip px-3 py-2 bg-muted flex items-center gap-2">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-primary text-primary-foreground border-2 border-border-strong ml-auto flex-shrink-0">
                      {c.calcLabel}
                    </span>
                  </div>
                        ...
                        <Fragment key={si}>
                          {sec.title && (
                            <tr>
                              <td
                                colSpan={4}
                                className="px-3 pt-2 pb-1 text-[10px] uppercase tracking-wide text-muted-foreground bg-muted/30"
                              >
                                {sec.title}
                              </td>
                            </tr>
      ...
      <div className="flex flex-wrap justify-end items-center gap-2 mb-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              <Download className="w-4 h-4" /> {tr("financeWs.download")}{" "}
          ...
          <DropdownMenuContent align="end" className="w-72">
            <DropdownMenuItem onClick={exportExcel} className="flex-col items-start gap-0.5 py-2">
              <span className="flex items-center gap-2 font-medium">
                <FileSpreadsheet className="w-4 h-4" /> {tr("financeWs.excelThreeSheets")}
              </span>
              <span className="text-xs text-muted-foreground pl-6">
                {tr("financeWs.excelThreeSheetsDesc")}
              </span>
            </DropdownMenuItem>
            <DropdownMenuItem
            ...
            >
              <span className="flex items-center gap-2 font-medium">
                <Download className="w-4 h-4" /> {tr("financeWs.csvSummary")}
              </span>
              <span className="text-xs text-muted-foreground pl-6">
                {tr("financeWs.csvSummaryDesc")}
              </span>
            </DropdownMenuItem>
            <DropdownMenuItem
            ...
            >
              <span className="flex items-center gap-2 font-medium">
                <Download className="w-4 h-4" /> {tr("financeWs.csvDetail")}
              </span>
              <span className="text-xs text-muted-foreground pl-6">
                {tr("financeWs.csvDetailDesc")}
              </span>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <table className="w-full text-sm whitespace-nowrap">
          <thead className="bg-muted text-left">
            <tr>
              ...
              {enabledCols.has("order_count") && (
                <th className="text-right px-3">{tr("financeWs.order")}</th>
              )}
              {enabledCols.has("fee_rider") && (
                <th className="text-right px-3">{tr("financeWs.feeRiderLabel")}</th>
              )}
              {enabledCols.has("active_date") && (
                <th className="text-right px-3">{tr("financeWs.active")}</th>
              )}
              {enabledCols.has("deductions") &&
                dedTypes.map((ty) => (
                  <th key={ty} className="text-right px-3">
                    {ty}
                  </th>
                ))}
              {enabledCols.has("total_fee") && (
                <th className="text-right px-3">{tr("financeWs.totalFee")}</th>
              )}
              {enabledCols.has("remarks") && (
                <th className="px-3 min-w-[180px]">{tr("financeWs.remarks")}</th>
              )}
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td colSpan={visibleColCount} className="p-6 text-center text-muted-foreground">
                  {tr("financeWs.noDataEmptyState")}
                </td>
              </tr>
            ) : (
              paged.map((r) => (
                <Fragment key={r.detailId}>
                  <tr
                    className={`border-t border-border ${r.held ? "bg-destructive/5" : r.isKasbonRow ? "bg-secondary" : ""}`}
                  >
                    <td className="p-2 sticky left-0 bg-background">
                      <button
                        onClick={() =>
                          !r.isKasbonRow && setExpanded(expanded === r.detailId ? null : r.detailId)
                        }
                        className={`flex items-center gap-1.5 text-left ${r.isKasbonRow ? "cursor-default" : "hover:text-primary"}`}
                      >
                        {!r.isKasbonRow && (
                          <ChevronRight
                            className={`w-3.5 h-3.5 flex-shrink-0 transition-transform ${expanded === r.detailId ? "rotate-90" : ""}`}
                          />
                        )}
                        <span>
                          <span className="font-medium flex items-center gap-1.5">
                            ...
                            {r.held && (
                              <span
                                title={r.holdReason ?? ""}
                                className="rounded border-2 border-border-strong px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide bg-destructive text-destructive-foreground"
                              >
                            ...
                            {r.isKasbonRow && (
                              <span className="rounded border-2 border-border-strong px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide bg-primary text-primary-foreground">
                                {r.kasbonNoTransfer
                                  ? tr("financeWs.kasbonInternalBadge")
                                  : tr("financeWs.kasbonTransferBadge")}
                              </span>
                            )}
                          </span>
                          <span className="text-xs text-muted-foreground block">
                            {r.isKasbonRow ? r.remarks : r.employeeId}
                          </span>
                        </span>
                      </button>
                    </td>
                    {enabledCols.has("client") && (
                      <td className="px-3 text-[13px] text-muted-foreground">{r.clientName}</td>
                    )}
                    {enabledCols.has("order_count") && (
                      <td className="text-right px-3 tabular-nums">
                        {r.isKasbonRow ? "—" : r.orderCount}
                      </td>
                    )}
                    {enabledCols.has("fee_rider") && (
                      <td className="text-right px-3 tabular-nums">
                        {r.isKasbonRow ? "—" : rp(r.feeRider)}
                      </td>
                    )}
                    {enabledCols.has("active_date") && (
                      <td className="text-right px-3 tabular-nums">
                        {r.isKasbonRow ? "—" : r.activeDates}
                      </td>
                    )}
                    {enabledCols.has("deductions") &&
                      dedTypes.map((ty) => (
                        <td key={ty} className="text-right px-3 tabular-nums text-destructive">
                          {r.isKasbonRow ? "—" : r.ded[ty] ? rp(r.ded[ty]) : "—"}
                        </td>
                      ))}
                    {enabledCols.has("total_fee") && (
                          ...
                          <input
                            ...
                            onBlur={(e) => {
                              if (e.target.value !== r.remarks)
                                saveRemark(r.detailId, e.target.value);
                            }}
                            placeholder={tr("financeWs.remarksPlaceholder")}
                            className="w-full min-w-[160px] rounded border border-border bg-background px-2 py-1 text-xs"
                          />
                        )}
                      </td>
                    )}
          ...
          {rows.length > 0 && (
            <tfoot className="bg-muted font-semibold">
              <tr>
                ...
                {enabledCols.has("deductions") &&
                  dedTypes.map((ty) => (
                    <td key={ty} className="text-right px-3 tabular-nums">
                      {rp(t.ded[ty] ?? 0)}
                    </td>
                  ))}
                {enabledCols.has("total_fee") && (
      ...
      <p className="text-xs text-muted-foreground mt-2">{tr("financeWs.helpText")}</p>
      {heldRows.length > 0 && (
        <p className="text-xs text-destructive mt-1">
          {heldRows.length} {tr("financeWs.heldRidersPrefix")} (
          {rp(heldRows.reduce((s, r) => s + r.total, 0))}) {tr("financeWs.heldRidersSuffix")}
        </p>
      )}
      {rows.some((r) => r.isKasbonRow && !r.kasbonNoTransfer) && (
        <p className="text-xs text-muted-foreground mt-1">{tr("financeWs.kasbonTransferNote")}</p>
      )}
      {internalKasbonRows.length > 0 && (
        <p className="text-xs text-muted-foreground mt-1">
          {internalKasbonRows.length} {tr("financeWs.internalKasbonPrefix")} (
          {rp(internalKasbonRows.reduce((s, r) => s + r.total, 0))}){" "}
          {tr("financeWs.internalKasbonSuffix")}
        </p>
      )}
    </>
  );
...
function RiderDetail({ r }: { r: RiderRow }) {
  ...
  // Kolom City/Rule Area cuma muncul kalau emang ada isinya — client yang
  // City-nya selalu kosong atau gak pakai Area City Pricing gak usah liat
  // kolom kosong isinya "—" semua. City ditampilin independen dari Rule Area
  // (berguna buat cek "City-nya keisi gak" walau client belum pakai fitur
  // Area Pricing sama sekali).
  const hasCity = r.deliv.some((d) => d.city != null);
  const hasAreaRule = r.deliv.some((d) => d.areaRule != null);
  const baseColSpan = 5 + (hasCity ? 1 : 0) + (hasAreaRule ? 1 : 0);
  return (
    <div className="space-y-3">
      {r.deliv.length > 0 && (
        <div>
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">
            {t("financeWs.deliveriesHeading")} ({r.deliv.length})
          </div>
          <div className="overflow-x-auto rounded border border-border">
            <table className="w-full text-xs whitespace-nowrap bg-card">
              <thead className="bg-muted text-left">
                <tr>
                  <th className="px-3 py-1.5">{t("financeWs.date")}</th>
                  <th className="text-right px-3">{t("financeWs.distanceKm")}</th>
                  <th className="text-right px-3">{t("financeWs.weightKg")}</th>
                  <th className="px-3">{t("financeWs.district")}</th>
                  {hasCity && <th className="px-3">{t("financeWs.city")}</th>}
                  {hasAreaRule && <th className="px-3">{t("financeWs.areaRule")}</th>}
                  <th className="px-3">{t("financeWs.type")}</th>
                  <th className="text-right px-3">{t("financeWs.fee")}</th>
                </tr>
              </thead>
              <tbody>
                {r.deliv.map((d, i) => (
                  <tr key={i} className="border-t border-border">
                    ...
                    {hasCity && <td className="px-3">{d.city ?? "—"}</td>}
                    {hasAreaRule && <td className="px-3">{d.areaRule ?? "—"}</td>}
                ...
                <tr className="border-t border-border-strong font-medium">
                  <td className="px-3 py-1.5" colSpan={baseColSpan}>
                    {t("financeWs.subtotalDeliveries")}
                  </td>
                  <td className="text-right px-3 tabular-nums">{rp(delivSum)}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      {r.att.length > 0 && (
        <div>
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">
            {t("financeWs.attendanceHeading")} ({r.att.length})
          </div>
          <div className="overflow-x-auto rounded border border-border">
            <table className="w-full text-xs whitespace-nowrap bg-card">
              <thead className="bg-muted text-left">
                <tr>
                  <th className="px-3 py-1.5">{t("financeWs.date")}</th>
                  <th className="px-3">{t("financeWs.clockIn")}</th>
                  <th className="px-3">{t("financeWs.clockOut")}</th>
                  <th className="px-3">{t("financeWs.duration")}</th>
                  <th className="px-3">{t("financeWs.otp")}</th>
                  <th className="text-right px-3">{t("financeWs.fee")}</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr className="border-t border-border-strong font-medium">
                  <td className="px-3 py-1.5" colSpan={5}>
                    {t("financeWs.subtotalAttendance")}
                  </td>
                  <td className="text-right px-3 tabular-nums">{rp(attSum)}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <p className="text-xs text-muted-foreground">
        Σ {t("financeWs.deliveriesShort")} {rp(delivSum)} + {t("financeWs.attendanceShort")}{" "}
        {rp(attSum)} = {t("financeWs.feeRiderLabel")} {rp(r.feeRider)} →{" "}
        {t("financeWs.minusDeductions")} → {t("financeWs.totalLabel")} {rp(r.total)}.
      </p>
    </div>
  );
}
