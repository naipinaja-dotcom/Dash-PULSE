...
export function PageSizeSelect({ pageSize, setPageSize }: { pageSize: number; setPageSize: (n: number) => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <label className="text-xs text-muted-foreground whitespace-nowrap">{t("paginationBar.show")}</label>
      <select
      ...
      >
        ...
        <option value="custom">{t("paginationBar.custom")}{!isPreset ? ` (${pageSize})` : ""}</option>
      </select>
      {customOpen && (
        <input
          type="number"
          min={1}
          autoFocus
          placeholder={t("paginationBar.amount")}
...
export function PaginationBar({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 mt-3 text-sm text-muted-foreground">
      <span>{t("paginationBar.showing")} {from}–{to} {t("paginationBar.of")} {total}</span>
      <div className="flex items-center gap-2">
        <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
          ...
          {t("paginationBar.previous")}
        </button>
        <span>{t("paginationBar.page")} {page} / {totalPages}</span>
